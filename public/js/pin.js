function fetchPin(pin, postoffice) {
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (data.length )
            {
                ...
                if(postoffice && postoffice != 'undefined'){
                  //  alert(postoffice);
                    $('#post_office_select').val(postoffice).change()
                }
...
function fetchPinPublic(pin) {
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            {
                $.each(data, function (index, value) {
                   $("#post_office").append('<option value="'+value.post_office_name +'">'+value.post_office_name+'</option>');
                });
