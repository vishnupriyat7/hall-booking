...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.recommending-offices.update", [$recommendingOffice->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.recommendingOffice.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $recommendingOffice->name) }}">
                @if($errors->has('name'))
