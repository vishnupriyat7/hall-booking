...
@can('visiting_office_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.visiting-offices.create') }}">
                        ...
                        <tr data-entry-id="{{ $visitingOffice->id }}">
                            ...
                            <td>
                                @can('visiting_office_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.visiting-offices.show', $visitingOffice->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('visiting_office_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.visiting-offices.edit', $visitingOffice->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('visiting_office_delete')
                                    <form action="{{ route('admin.visiting-offices.destroy', $visitingOffice->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('visiting_office_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.visiting-offices.massDestroy') }}",
