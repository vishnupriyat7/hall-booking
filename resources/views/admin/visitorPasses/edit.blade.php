    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.visitor-passes.update", [$visitorPass->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('visiting_office_category') ? 'is-invalid' : '' }}" name="visiting_office_category_id" id="visiting_office_category_id">
                    @foreach($visiting_office_categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('visiting_office_category_id') ? old('visiting_office_category_id') : $visitorPass->visiting_office_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('visiting_office') ? 'is-invalid' : '' }}" type="text" name="visiting_office" id="visiting_office" value="{{ old('visiting_office', $visitorPass->visiting_office) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('recommending_office_category') ? 'is-invalid' : '' }}" name="recommending_office_category_id" id="recommending_office_category_id">
                    @foreach($recommending_office_categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('recommending_office_category_id') ? old('recommending_office_category_id') : $visitorPass->recommending_office_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('recommending_office') ? 'is-invalid' : '' }}" type="text" name="recommending_office" id="recommending_office" value="{{ old('recommending_office', $visitorPass->recommending_office) }}">
