...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.locker.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lockers.update", [$locker->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pass') ? 'is-invalid' : '' }}" name="pass_id" id="pass_id">
                    @foreach($passes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pass_id') ? old('pass_id') : $locker->pass->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('pass'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('token') ? 'is-invalid' : '' }}" type="text" name="token" id="token" value="{{ old('token', $locker->token) }}">
                @if($errors->has('token'))
