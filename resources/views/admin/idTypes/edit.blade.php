...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.id-types.update", [$idType->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.idType.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $idType->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('min_length') ? 'is-invalid' : '' }}" type="number" name="min_length" id="min_length" value="{{ old('min_length', $idType->min_length) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max_length') ? 'is-invalid' : '' }}" type="number" name="max_length" id="max_length" value="{{ old('max_length', $idType->max_length) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('regex') ? 'is-invalid' : '' }}" type="text" name="regex" id="regex" value="{{ old('regex', $idType->regex) }}">
                @if($errors->has('regex'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_mal') ? 'is-invalid' : '' }}" type="text" name="name_mal" id="name_mal" value="{{ old('name_mal', $idType->name_mal) }}">
