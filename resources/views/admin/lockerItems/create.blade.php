...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.lockerItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.locker-items.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('locker') ? 'is-invalid' : '' }}" name="locker_id" id="locker_id" required>
                    @foreach($lockers as $id => $entry)
                        <option value="{{ $id }}" {{ old('locker_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
