    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.districts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('state_cd') ? 'is-invalid' : '' }}" name="state_cd_id" id="state_cd_id">
                    @foreach($state_cds as $id => $entry)
                        <option value="{{ $id }}" {{ old('state_cd_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('district_abbr') ? 'is-invalid' : '' }}" type="text" name="district_abbr" id="district_abbr" value="{{ old('district_abbr', '') }}">
