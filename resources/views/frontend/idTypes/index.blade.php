        ...
        <div class="col-md-12">
            @can('id_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.id-types.create') }}">
                                ...
                                @foreach($idTypes as $key => $idType)
                                    <tr data-entry-id="{{ $idType->id }}">
                                        ...
                                        <td>
                                            ...
                                            @can('id_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.id-types.edit', $idType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('id_type_delete')
                                                <form action="{{ route('frontend.id-types.destroy', $idType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('id_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.id-types.massDestroy') }}",
